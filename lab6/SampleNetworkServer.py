...
class SmartNetworkThermometer (threading.Thread) :
    ...
    def __init__ (self, source, updatePeriod, port, password) : # Added password as parameter.
                ...
                if cs[0] == "AUTH":
                    if cs[1] == self.hashed_password :
                        if len(self.tokens)>9: #Added Check for number of active tokens
                            ...
                            self.serverSocket.sendto(b"Too many active tokens. Please Logout a session.", addr)
                        else:
...
bobThermo = SmartNetworkThermometer(bob, UPDATE_PERIOD, 23456,password)
...
incThermo = SmartNetworkThermometer(inc, UPDATE_PERIOD, 23457,password)
